...
private:
    ...
    olc::Sprite *pWindowSprite  = nullptr;

    float fFloorReflection = 0.0f;    // 0.0f = no reflection, 1.0f = full mirror
...
public:
    bool OnUserCreate() override {
        ...
        pFloorSprite = new olc::Sprite( sSpritePath + "floor1 - variant.png" );
//        pFloorSprite = new olc::Sprite( sSpritePath + "bluestone.png" );
        pCeilSprite  = new olc::Sprite( sSpritePath + "wood.png" );

        // prepare a sprite for rendering windows - copy wall sprite and render window on top of it
        olc::Sprite *pTmpSprite = new olc::Sprite( sSpritePath + "window.rbg.png" );
        pWindowSprite = pWallSprite->Duplicate();
        SetDrawTarget( pWindowSprite );
        DrawSprite( 25, 25, pTmpSprite );   // there are assumptions on the window sprite here!!
    ...
    bool GetDistanceToWall( float fRayAngle, float &fHitX, float &fHitY, float &fDistIfFound, int &nMapCoordX, int &nMapCoordY, int &nType ) {
        ...
        if (bHitFound) {
            ...
            nMapCoordX = nCurX;
            nMapCoordY = nCurY;
            // return the type of wall that was hit
            switch (sMap[ nCurY * nMapX + nCurX ]) {
                case '#': nType = 0; break;
                case 'W': nType = 1; break;
            }
        } else {
    ...
    bool OnUserUpdate( float fElapsedTime ) override {
        ...
        // manipulate floor reflection
        if (GetKey( olc::NP_ADD ).bHeld) { fFloorReflection += 0.1f * fElapsedTime; if (fFloorReflection > 1.0f) fFloorReflection = 1.0f; }
        if (GetKey( olc::NP_SUB ).bHeld) { fFloorReflection -= 0.1f * fElapsedTime; if (fFloorReflection < 0.0f) fFloorReflection = 0.0f; }
        ...
        // collision detection - check if within map bounds and inside empty tile
        ...
        for (int x = 0; x < ScreenWidth(); x++) {
            ...
            if (GetDistanceToWall( fCurAngle, fX_hit, fY_hit, fRawDistToWall, nX_hit, nY_hit, nWallType )) {
            ...
            for (int y = 0; y < ScreenHeight(); y++) {
                ...
                // calculate the world floor coordinate from the distance and the view angle + player angle
                    ...
                    // get the sample to make the floor reflect
                    int nSampleY2 = nWallFloor - (y - nWallFloor);
                    olc::Sprite *tmp = GetDrawTarget();
                    olc::Pixel reflect = tmp->GetPixel( x, nSampleY2 );

                    // having both sample coordinates, get the floor sample
                    olc::Pixel auxSample = pFloorSprite->Sample( fSampleX, fSampleY );
                    // blend it with the reflect sample to create a reflective floor effect
                    olc::Pixel blended = PixelLerp( auxSample, reflect, fFloorReflection );
                    Draw( x, y, blended );
                    ...
                    // having both sample coordinates, get the sample and draw the pixel
                    olc::Sprite *pSelectWallType = nullptr;
                    switch (nWallType) {
                        case 0: pSelectWallType = pWallSprite; break;
                        case 1: pSelectWallType = pWindowSprite; break;
                        case -1:
                        default: std::cout << "SHIT HERE" << std::endl;
                    }
                    olc::Pixel auxSample = pSelectWallType->Sample( fSampleX, fSampleY );
                    Draw( x, y, auxSample );
                }
            }
        }
        ...
        DrawString( 10, 40, "reflection = " + std::to_string( fFloorReflection ), COL_TEXT );
