...
//
// Implementation of part 24a - isolated (in separate module) RC_Misc.h and .cpp
//
...
/* Short description
   -----------------
   This implementation is a follow up of implementation part 23 g. See the description of that implementation as well (and check on
   the differences between that cpp file and this one).

   A number of miscellaneous constants and conversion functions are put into a separate module RC_Misc. This includes
   the lookup functions for fast sin() and cos() values. 
   
   A renaming is done of the word Block (denoting a 3D cell in the map) to Map Cell, throughout the code.

   Functions rad_mod2pi() and deg_mod360() are renamed since its implicit that 2 pi is in radians and 360 is in degrees.
 */
