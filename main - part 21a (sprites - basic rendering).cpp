...
/* Short description
   -----------------
   This implementation is a follow up of implementation part 20. See the description of that implementation as well (and check on
   the differences between that cpp file and this one).

   I used the technique/implementation from Javidx9's FPS video part 2.
   (url: https://www.youtube.com/watch?v=HEb2akswCcw - Thanks @Javidx9 !)
     * Added a struct sObject, and a list of objects as class variables.
     * Added sprite pointer for the object, and inited it in OnUserCreate()
     * Created a global list of objects and populated it in OnUserCreate()
     * Added rendering code after rendering of background scenery and before rendering of minimap and debug output
     * Added a rudimentary form of masking for the objects

   Oh and I also added converter functions to convert between degree and radian valued angles (and vice versa).
...
#define FLOOR_3QRTR '3'    //                 3/4
    ...
    bool OnUserCreate() override {
        ...
        vListObjects = {
            { 10.5f,  4.5f, pObjectSprite },
            { 14.5f,  6.5f, pObjectSprite },
            {  6.5f,  9.5f, pObjectSprite },
        };
...
#define    ROOF_DRAWING 4
        ...
        for (auto &object : vListObjects) {
            ...
            // "bodge" angle in range [ -PI, PI ]
            if (fObjA < - PI) fObjA += 2.0f * PI;
            if (fObjA >   PI) fObjA -= 2.0f * PI;

            bool bInFOV = fabs( fObjA ) < fPlayerFoV_rad / 2.0f;
            ...
            if (bInFOV && fDistanceFromPlayer >= 0.3f && fDistanceFromPlayer < fMaxDistance) {
                // work out where objects floor and ceiling are (in screen space)
                float fObjCeiling = (float)(ScreenHeight() / 2.0f) - (float)(ScreenHeight() / fDistanceFromPlayer);
                // this is by symmetry around horizon
                float fObjFloor = ScreenHeight() - fObjCeiling;
