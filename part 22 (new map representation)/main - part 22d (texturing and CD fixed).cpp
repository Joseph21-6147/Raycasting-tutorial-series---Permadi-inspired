...
//
// Implementation of part 22 d - holes, overhangs and floating blocks (roof and ceiling bugs fixed)
//
...
/* Short description
   -----------------
   This implementation is a follow up of implementation part 22 c. See the description of that implementation as well (and check on
   the differences between that cpp file and this one).

     * After the rebuild of the rendering mechanism in the previous version, this code only contains bug fixes for rendering the roofs and
       ceilings. Check the Permadi tutorial and make your own sketch with pen and paper to comprehend the fixes.
...
// test objects
#define NR_TEST_OBJECTS        0      // switched off to focus on wall rendering
    ...
    bool OnUserUpdate( float fElapsedTime ) override {
        ...
        for (int x = 0; x < ScreenWidth(); x++) {
            ...
            auto get_roof_sample = [=]( int px, int py, int nLevel, float fHeightWithinLevel ) -> olc::Pixel {
                ...
                // (the pixel is given since you know the x and y to draw to)
                float fRoofProjDistance = (( (fPlayerH - (float( nLevel ) + fHeightWithinLevel)) / float( py - nHorizonHeight )) * fDistToProjPlane) / lu_cos( fViewAngle );
                // call the generic sampler to work out the rest
            ...
            auto get_ceil_sample = [=]( int px, int py, int nLevel, float fHeightWithinLevel ) -> olc::Pixel {
                ...
                float fCeilProjDistance = (( (float( nLevel ) - fPlayerH) / float( nHorizonHeight - py )) * fDistToProjPlane) / lu_cos( fViewAngle );
                // call the generic sampler to work out the rest
