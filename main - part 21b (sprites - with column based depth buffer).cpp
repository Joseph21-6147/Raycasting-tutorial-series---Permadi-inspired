...
//
// Implementation of part 21 b - introducing sprites (billboards): added simple depth buffer
//
...
/* Short description
   -----------------
   This implementation is a follow up of implementation part 21 a. See the description of that implementation as well (and check on
   ...
   (url: https://www.youtube.com/watch?v=HEb2akswCcw - Thanks @Javidx9 !)
     ...
     * Added a simple (column based) depth buffer (declaration, initialization, updating and application)
                ...
                for (float fx = 0.0f; fx < fObjWidth; fx++) {
                    ...
                    if (nObjColumn >= 0 && nObjColumn < ScreenWidth()) {
                        for (float fy = 0.0f; fy < fObjHeight; fy++) {
                            ...
                            if (pSample != olc::BLANK && fDepthBuffer[nObjColumn] >= fDistanceFromPlayer) {
