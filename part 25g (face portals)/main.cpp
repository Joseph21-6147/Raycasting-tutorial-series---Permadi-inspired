...
//
// Implementation of part 25 g - portal characteristic brought to faces (instead of map cells)
//
...
/* Short description
   -----------------
   This implementation is a follow up of implementation part 25 f. See the description of that implementation as well (and check on
   the differences between that cpp file and this one).

   Summary of the changes I made:
     * main.cpp
         + I put the screen and pixel constants into a separate include file RC_Screen.h, so that the map definition file could use them.
         + Needed to keep track of previous location to work out if a portal face was crossed, so implemented cached variants on player position variables. These are updated on top of OnUserUpdate()
         + In OnUserCreate() I now use player location and orientation values that are from the user defined map file. This way I don't need to change these values every time I use another map definition file.
         + Adapted the dda function to terminate the loop if a portal face was found
         + I gave portals also the possibility of having another orientation "at the other side" - i.e. the player going into the portal going east and leaving the portal in another map going north for instance. However this functionality doesn't work correctly (yet).
         + The RenderMap() is adapted to show the portals as faces (instead of map cells)
     * map_16x16.h
         + The portals are having a distinct direction now, so I defined four of them (and rearranged the symbols for the map cell blue prints a bit as a result).
         + Added the (potentially different) orientation angle to the portal descriptor blueprints per map
     * RC_Face
         + The face blue prints are extended with an additional flag to denote if it's a portal or not.
         + I also gave all the face classes a method IsPortal(), and defined a new RC_FacePortal derived class
     * RC_Face_blueprints.cpp
         + All the face blueprints needed another bool flag, so added that
     * RC_Map
         + Removed to code that dealt with portal map cells, since they no longer exist
         + Added code to deal with portal faces
     * RC_MapCell
         + Added an orientation angle to the portal descriptor record
         + Removed the RC_MapCelPortal class, and all the IsPortal() methods from the RC_MapCell family
     * RC_MapCell_blueprint.cpp
         + Removed the flag that used to denote portalness of the map cell
         + Created new map cells to model directed portals, using the portal face
...
//            }
    ...
    bool CastRayPerLevelAndAngle( int nCurMap, float fPx, float fPy, int layer, float fRayAngle_deg, std::vector<IntersectInfo> &vHitList ) {
        ...
        bool bPortalFaceFound = false;

        // terminate the loop / algorithm if out of bounds or destination found or maxdistance exceeded
        while (!bOutOfBounds && !bDestCellReached && !bPortalFaceFound && fDistIfFound < fMaxDistance) {
            ...
            if (hitRec.fHeight > 0.0f) {
                ...
                if (auxFacePtr->IsPortal()) {
                    ...
                    if (nStopY > nStrtY) {
                        ...
                        int nDeltaX = auxPortalFacePtr->GetToX() - int( hitRec.fHitX );
                        int nDeltaY = auxPortalFacePtr->GetToY() - int( hitRec.fHitY );
                        ...
                        switch ( auxPortalFacePtr->GetExitDir() ) {
                            ...
                            case FACE_EAST : fOtherX = auxPortalFacePtr->GetToX()           ; fOtherY = hitRec.fHitY + nDeltaY;        break;
                            case FACE_WEST : fOtherX = auxPortalFacePtr->GetToX() + 0.99999f; fOtherY = hitRec.fHitY + nDeltaY;        break;
                            case FACE_SOUTH: fOtherX = hitRec.fHitX + nDeltaX;        fOtherY = auxPortalFacePtr->GetToY()           ; break;
                            case FACE_NORTH: fOtherX = hitRec.fHitX + nDeltaX;        fOtherY = auxPortalFacePtr->GetToY() + 0.99999f; break;
                            default: std::cout << "ERROR: RenderSubSlice() --> this exit direction doesn't implement" << std::endl;
                        }

                        RenderSubSlice(
                            nRecursionDepth + 1,
                            auxPortalFacePtr->GetExitDir(),
                            fViewAngle_deg,
                            fCurAngle_deg,

                            auxPortalFacePtr->GetToMap(),
                            fOtherX, fOtherY, fPh,
