...
//
// Implementation of part 23 e - dynamic, animated and wall objects (animation of doors or gates)
//
// Joseph21, may 3, 2023
//
// Dependencies:
...
/* Short description
   -----------------
   This implementation is a follow up of implementation part 23 d. See the description of that implementation as well (and check on
   the differences between that cpp file and this one).

   Needed for animated objects:
     * Added flags bPermeable and bAnimated to BlockType. Permable denotes if a block can be passed by the player or should be
       considered solid. Animated speaks for itself.
     * Created a class RC_Animation to implement animated block objects. You can init this class, set it's state. Important methods
       are Sample() to get a pixel from an animated object, and Update() to implement the animation using fElapsedTime.
     * I added a vector with animated objects (or nullptr if there aren't) to the map, as a separate level on top of charachter map
       and height map.
     * Added some test code in OnUserUpdate() to test on the animation of door/gate objects. This is implemented in the game logic
       phase of OnUserUpdate()
     * Had to adapt the wall rendering due to introduction of animated objects.

   Smaller stuff:
     * the number of test objects used to be defined as a constant int. Now I define it as a percentage, with interpretation that
       the percentage of the empty tiles in the map will be the number of objects to create. This is easier when scaling map sizes.
     * Added the number of objects in the debug player info hud rendering
...
void AddBlock( char cID, float fH, int nE, int nN, int nW, int nS, int nT, int nB, bool bT = false, bool bP = false, bool bA = false ) {
    BlockType sTmpBlock = { cID, fH, nE, nN, nW, nS, nT, nB, bT, bP, bA };
...
void InitBlockTypes() {
  ...
  AddBlock( '$', 1.00f, 4, 4, 4, 4, 0, 0, true, false, true );   // door / gate
...
public:
    ...
    void InitAnimation( int px, int py, int l, int s, olc::Sprite *spr, int tw, int th, int tx, int ty ) {
        x = px;
        y = py;
        level = l;
        state = s;
        ...
        if (fTimer >= fThreshold) {
            ...
            if (nCounter == nThreshold) {
                ...
                switch (state) {
                    ...
                    case ANIM_STATE_CLOSING: tileX -= 1; break;
    ...
    olc::Pixel Sample( float sX, float sY ) {
        //
        int   sx0 = ( tileX + sX ) * tileWidth;
        int   sy0 = ( tileY + sY ) * tileHight;
        float fx0 = float( sx0 ) / pSpriteSheet->width;
        float fy0 = float( sy0 ) / pSpriteSheet->height;

        return pSpriteSheet->Sample( fx0, fy0 );
    }

public:

    int x, y, level;    // the tile coordinate and level this animation belongs to (block)
    int state;          // one of above constants

    olc::Sprite *pSpriteSheet = nullptr;   // the sprite containing the animation sequence
...
public:
    ...
    void AddLayer( const std::string &sUserMap ) {
        ...
        std::vector<RC_Animation *> vLevelAnims;
        RC_Animation *pValue;
        ...
        for (int y = 0; y < nMapY; y++) {
            for (int x = 0; x < nMapX; x++) {
                ...
                if (refBlock.bAnimated) {

                    // THIS IS A HACK!!
                    olc::Sprite *sprSheet = new olc::Sprite( vWallSpriteFiles[ GetBlockFaceIx( refBlock, FACE_EAST )] );

                    pValue = new RC_Animation;
                    pValue->InitAnimation( x, y, (int)fMaps.size(), ANIM_STATE_CLOSED, sprSheet, 32, 32, 0, 0 );
                } else {
                    pValue = nullptr;
                }
                vLevelAnims.push_back( pValue );
            }
        }
        sMaps.push_back( sMap );
        fMaps.push_back( fMap );
        aMaps.push_back( vLevelAnims );
    }

    // method to clean up the object before it gets out of scope
    void FinalizeMap() {
        for (auto &elt : fMaps ) { delete elt; }
        for (auto &outer : aMaps ) {
    ...
    // getter for obtaining the pointer to the associated animation (can return nullptr) of the cell at level, coordinates (x, y)
    RC_Animation *AnimationPtrAt( int x, int y, int level ) {
        RC_Animation *result = nullptr;
        if (!IsInBounds( x, y )) {
            std::cout << "ERROR: AnimationPtrAt() --> map indices out of bounds: (" << x << ", " << y << ")" << std::endl;
        } else if (level >= (int)aMaps.size()) {
            std::cout << "ERROR: AnimationPtrAt() --> level argument out of range: " << level << std::endl;
        } else {
            result = aMaps[ level ][y * nMapX + x];
        }
        return result;
    }
    ...
    bool Collides( float fX, float fY, float fH, float fR, float fVX, float fVY ) {
        ...
        if (!IsInBounds( fX + fOffsetX, fY + fOffsetY ) || (fH - fR) < 0.0f) {
            ...
            char cBlockID = CellValueAt(int( fX + fOffsetX ), int( fY + fOffsetY ), int( fH ) );
            BlockType &refBlck = GetBlockType( cBlockID );
            bResult = (
...
private:
    ...
    std::vector<std::vector<RC_Animation *>> aMaps;   // contains pointers to RC_Animation objects for animated walls
        ...
        for (int y = 0; y < cMap.Hight(); y++) {
            for (int x = 0; x < cMap.Width(); x++) {
                for (int h = 0; h < cMap.NrOfLayers(); h++) {
                    RC_Animation *aux = cMap.AnimationPtrAt( x, y, h );
                    if (aux != nullptr) {

                        if (bStateChanged) {
                            aux->SetState( nTestAnimState );
                        } else {
                            aux->UpdateAnimation( fElapsedTime );
                            // make sure that (only) fully opened animated blocks are permeable
                            char cBlckId = cMap.CellValueAt( x, y, h );
                            BlockType &refBlck = GetBlockType( cBlckId );
                            refBlck.bPermeable = aux->state == ANIM_STATE_OPENED;
                        }
                    }
                }
            }
            ...
            for (auto &hitRec : vHitPointList) {
                ...
                if (hitRec.fHeight > 0.0f) {
                    ...
                    for (int y = nWallTop; y <= nWallBot; y++) {
                        ...
                        olc::Pixel wallSample;
                        // if we hit an animated block, the sampling is a bit different, since we must sample a sprite sheet depending on the animation state and parameters
                        if (sBlockHit.bAnimated) {
                            RC_Animation *pAnim = cMap.AnimationPtrAt( nX_hit, nY_hit, nBlockLevel );
                            if (pAnim == nullptr) {
                                std::cout << "ERROR: OnUserUpdate() --> nullptr to animation object at ( " << nX_hit << ", " << nY_hit << ", " << nBlockLevel << ")" << std::endl;
                                wallSample = olc::MAGENTA;
                            } else {
                                // obtain the sprite sheet pointer
                                // determine the sub sprite / tile coordinates in the sprite sheet
                                // sample the sub sprite
                                wallSample = ShadePixel( pAnim->Sample( fSampleX, fSampleY ), fFrntDistance );
                            }
                        } else {
                            ...
                            wallSample = ShadePixel( pSpritePtr->Sample( fSampleX, fSampleY ), fFrntDistance );
                        }
