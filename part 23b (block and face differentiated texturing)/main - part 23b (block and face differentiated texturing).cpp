...
//
// Implementation of part 23 b - dynamic, animated and wall objects (block and face differentiated texturing)
//
...
//   *  include file "map_10x10 - part 23b.h" - contains the map definitions and the sprite file names

/* Short description
   -----------------
   This implementation is a follow up of implementation part 23 a. See the description of that implementation as well (and check on
   the differences between that cpp file and this one).


     * Introduced a block type object that contains the configs for a block - sprite indices for walls and roof/ceiling, height, and
       character ID in the map. This makes it possible to differentiate blocks, and even differentiate faces per block when texturing.
       I coded a little library for easy access on the block type using the character ID. This also made a simplification of the map
       initialisation possible.
     * Replaced the singular pointers to "the one" wall sprite, "the one" floor sprite etc with vectors of pointers to a library of wall,
       floor, roof, ceiling and objects sprites. Rearranged OnUserCreate() for the sprite loading: A vector of file names is initialised
       in the map definition file. This is done for all the sprite categories (wall texturing, ceiling, roof, floor texturing and object
       sprites). These vectors are used to populate vectors with sprite pointers. These function as libraries, and are indexed by the
       indices that are stored in the block type.
     * Had to rebuild the lambda for roof and ceiling texturing, since it must select the sprite from the block type info now. The same
       adaptation is made for selecting the sprite for rendering the wall segments.

     * Smaller changes:
         + Added collision detection for the dynamic (moving) objects, so that they bounce off walls as well as off map boundaries. Added
           a method Collides() to RC_Maps to this end.
         + reduced screen size to keep a bit of performance going :|
         + made minimap size dependent of pixel size - otherwise it gets too large with larger pixel sizes
         + added object angle and speed to the object class (for rendering objects in the minimap)
         + added object drawing in the minimap
         + added possibility to rotate with to a discrete angle, using the num pad keys;
    ...
    void AddLayer( const std::string &sUserMap ) {
        ...
        for (int y = 0; y < nMapY; y++) {
            for (int x = 0; x < nMapX; x++) {
                BlockType &refBlock = GetBlock( sMap[ y * nMapX + x ] );
    ...
    bool OnUserUpdate( float fElapsedTime ) override {
        ...
        for (int x = 0; x < ScreenWidth(); x++) {
            ...
            auto generic_sampling_new = [=]( float fProjDistance, int nLevel, int nFaceID ) -> olc::Pixel {
                ...
                BlockType &sBlockHit = GetBlock( cBlockType );
...
//                    fBackDistance   = hitRec.fDistBack;
                    ...
                    for (int y = nWallTop; y <= nWallBot; y++) {
                        ...
                        BlockType &sBlockHit = GetBlock( cBlockType );
                        int nWallSpriteIx = GetBlockFaceIx( sBlockHit, nSide );
