...
//
// Implementation of part 25 e - extended checking of input data (user defined maps)
//
...
/* Short description
   -----------------
   This implementation is a follow up of implementation part 25 d. See the description of that implementation as well (and check on
   the differences between that cpp file and this one).

   Summary of the changes I made:
      * main.cpp
         + renamed fLookUp to fPlayerLU to emphasize that its a player characteristic
         + added std::vectors of wall, ceiling and roof sprites as parameters to InitFaceBluePrint()
         + improved the way the current map is determined for test mode output
         + removed the check on nullptr map cells in the map, since this check is done at startup time of the game once
      * RC_Face
         + added std::vectors with wall, ceiling and roof sprites as parameters to both AddFaceBluePrint() and InitFaceBluePrints()
         + Changed the order of the cardinal directions to be clockwise, starting from EAST
         + Added checks on face blue print input data
      * RC_Map
         + Added checks on input data for maps
         + Added "after assembly integrity check" on layers in AddLayer(): no nullptr map cells are allowed, and the only situation where
           nullptr faces are allowed is on empty maps (where all faces should be nullptr)
      * RC_MapCell
         + Added checks on input data for map cell blue prints
      * RC_MapCell_blueprint.cpp
         + Needed adaptation due to changed ordering of cardinal directions (north swapped with south)
...
public:
    ...
    // This function uses pCurMap as the map to cast the ray, and it casts the ray
        ...
        for (auto &hitRec : vHitPointList) {
            ...
            if (hitRec.fHeight > 0.0f) {
                ...
                for (int y = nOspBotFrnt + 1; y <= nOspBotBack; y++) {
                    ...
                    // either render or store for later rendering, depending on face transparency
                    RC_Face *auxFacePtr = auxMapCellPtr->GetFacePtr( FACE_BOTTOM );
                    if (auxFacePtr->IsTransparent()) {
