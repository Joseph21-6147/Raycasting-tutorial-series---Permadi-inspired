...
void RC_Object::Update( RC_Map &curMap, float fElapsedTime ) {
    if (!bStationary) {
        float newX = x + vx * fElapsedTime;
        float newY = y + vy * fElapsedTime;
        if (!curMap.Collides( newX, y, scale, RADIUS_ELF, vx, vy )) {
            x = newX;
        } else {
            vx = -vx;
            UpdateObjAngle();
        ...
        if (!curMap.Collides( x, newY, scale, RADIUS_ELF, vx, vy )) {
            y = newY;
        } else {
            vy = -vy;
            UpdateObjAngle();
