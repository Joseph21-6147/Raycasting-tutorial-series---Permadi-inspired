...
//
// Implementation of part 25 d - working version of portals with recursive sub slice rendering
//
...
/* Short description
   -----------------
   This implementation is a follow up of implementation part 25 c. See the description of that implementation as well (and check on
   the differences between that cpp file and this one).

   Summary of the changes I made:
      * parameterized CalculateBlockProjections() with viewpoint height (it used to grab class variable directly, causing me to pull out
        my hair :| )
      * All the wall, ceiling and roof rendering code is put into a separate method RenderSubSlice(). This method renders one slice of the screen
        at a time, and is called ScreenWidth() times per frame. If a portal is encountered, the method is called recursively, using the other map
        info and a narrower sub slice
      * The test code to hard coded move to another map is not needed anymore, and is removed
...
// ==============================/   Mini map rendering prototypes   /==============================
    ...
    void RenderSubSlice(
        ...
        if (bTestMode && nSlice == int( fTestSlice )) {
            std::string sMapID = "unknown";
            if (pCurMap == &vMaps[0]) sMapID = "0";
            if (pCurMap == &vMaps[1]) sMapID = "1";

            std::cout << "Map: " << sMapID << std::endl;
        ...
        for (auto &hitRec : vHitPointList) {
            ...
            if (hitRec.fHeight > 0.0f) {
                ...
                if (auxMapCellPtr == nullptr) {
                    std::cout << "FATAL ERROR: RenderSubSlice() --> situation should not occur!!! " <<
                                 "nullptr map cell ptr detected at (" << hitRec.nHitX << ", " << hitRec.nHitY << ") layer " << hitRec.nLayer << std::endl;
                }
