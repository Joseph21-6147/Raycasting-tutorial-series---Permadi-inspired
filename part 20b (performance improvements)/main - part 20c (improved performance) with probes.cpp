...
//
// Implementation of part 20 c - fractional wall heights with improved performance and probes
//
...
//   *  MuTimer.h and .cpp - timing and profiling library (not included)
...
/* Short description
   ...
   This is in fact just a copy of version 20 b, but this one has probes in it. Therefore it needs the MuTimer functionality.
...
#define FLOOR_3QRTR 'T'    //                 3/4
    ...
    bool OnUserCreate() override {
        ...
        std::vector<std::string> vProbeNames = {
            "user input",
            "rndr loop x   - DDA part",
            "rndr loop x   - coll. hit point data",
            "rndr loop x+y - coll. data f. cur pixel",
            "rndr loop x+y - rendering cur pixel",
            "rndr minimap",
            "rndr debuginfo",
            "engine time"
        };
        cProfiler.InitProbes( vProbeNames );
...
cProfiler.Probe( 7 );
    ...
    bool OnUserDestroy() {
        cProfiler.PrintStats( "profiling stats for OnUserUpdate() and peripheral PGE time", true );
