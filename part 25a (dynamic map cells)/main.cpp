...
//
// Implementation of part 25 a - dynamic (= moving height) map cells
//
...
/* Short description
   -----------------
   This implementation is a follow up of implementation part 24 h. See the description of that implementation as well (and check on
   the differences between that cpp file and this one).

   The additional code for this version aims at introducing the dynamic map cell. This is a type of map cell for which the height is
   constantly updated. NOTE: this should be considered a prototype, the details of this map cell type could be elaborated further.

   To implement this dynamic map cell, I:
     + Added bDynamic flag to map cell blueprint structure
     + Added bool IsDynamic() method to base RC_MapCell class
     + Added derived RC_MapCellDynamic class
     + Changed AddLayer() in RC_Map.cpp to account for new type of map cells
     + Changed map_16x16.h to test the dynamic map cell

   Furthermore I implemented some small improvements:
     + renamed level to layer in RC_Map.cpp
     + Renamed member variables of IntersectInfo in main.cpp
...
public:
    ...
    // The start position (fPx, fPy) is the "from point", a "to point" is determined using fRayAngle and fMaxDistance.
    ...
    bool CastRayPerLevelAndAngle( float fPx, float fPy, int layer, float fRayAngle_deg, std::vector<IntersectInfo> &vHitList ) {
    ...
    bool OnUserUpdate( float fElapsedTime ) override {
        ...
        for (int x = 0; x < ScreenWidth(); x++) {
            ...
            for (int k = 0; k < cMap.NrOfLayers(); k++) {
                ...
                CastRayPerLevelAndAngle( fPlayerX, fPlayerY, k, fCurAngle_deg, vCurLevelList );
