...
//
// Implementation of part 22 a - holes, overhangs and floating blocks (rebuild of map representation in class RC_Map)
//
// Joseph21, april 21, 2023
//
// Dependencies:
...
/* Short description
   -----------------
   This implementation is a follow up of implementation part 21 f. See the description of that implementation as well (and check on
   the differences between that cpp file and this one).

   In preparation of the alternative rendering method I added a class RC_Map that has the new map representation. Instead of one 2D
   datastructure that represents heights, I have the maps modelled now in a layered fashion, so that I can approach the layers separately
   for the raycasting.
...
public:
    ...
    float CellHeight( int x, int y ) {
        ...
        if (x < 0 || x >= nMapX || y < 0 || y >= nMapY) {
            ...
            result = fMap[ y * nMapX + x ];
        }
        return result;
    }

    bool IsInBounds( float x, float y ) {
        return (x >= 0.0f && x < nMapX &&
                y >= 0.0f && y < nMapY);
    }

private:
    std::string sMap;         // contains chars that define the type of block per map location
    float *fMap = nullptr;    // contains floats that represent the height per block
    int nMapX;
    int nMapY;
};
...
public:
    ...
    bool OnUserCreate() override {
        ...
        // tile layout of the map - string argument must match x and y dimensions!
        cMap.InitMap( 48, 48,
//            0         1         2         3         4         5
//            0123456789012345678901234567890123456789012345678901
        ...
        fMaxDistance = sqrt( cMap.Width() * cMap.Width() + cMap.Hight() * cMap.Hight() );
    ...
    bool OnUserUpdate( float fElapsedTime ) override {
        ...
        for (int x = 0; x < ScreenWidth(); x++) {
            ...
            if (GetDistancesToWalls( fCurAngle, vHitPointList )) {
                ...
                fCurDistance = vHitPointList[0].fDstFrnt;
                ...
                if (y >= nWallBottom) {
                    ...
                    while (nDrawMode == UNKNOWN_DRAWING) {
                        if (nHitListIndex < (int)vHitPointList.size() - 1) {
                            ...
                            fCurDistance = vHitPointList[ nHitListIndex ].fDstFrnt;
