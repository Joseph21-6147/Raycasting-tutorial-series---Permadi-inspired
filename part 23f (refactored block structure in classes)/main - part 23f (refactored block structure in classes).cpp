...
//
// Implementation of part 23 f - dynamic, animated and wall objects (class RC_Objects introduced)
//
// Joseph21, may 3, 2023
//
// Dependencies:
...
//   *  include file "map_16x16 - part 23e.h" - contains the map definitions and the sprite file names

/* Short description
   -----------------
   This implementation is a follow up of implementation part 22 e. See the description of that implementation as well (and check on
   the differences between that cpp file and this one).

     * Block library is more explicitly renamed bo Block BluePrint library - to make place for several block class types. Note: due to this
       renaming a lot of code appears to have changed, but that's just the renaming.
     * Implemented RC_Block class - a very basic empty block, just a placeholder really to prevent having to handle empty blocks
       differently then non empty blocks
     * Implemented derived class RC_BlockFilled - a non empty but otherwise static block.
     * Implemented derived class RC_BlockAnimated - a non empty and animated type block (such as doors etc).
     * The RC_Map object is heavily simplified, and doesn't contain separate layers for the characters, height per tile and objects anymore:
       all info is stored in the objects now, so the character and height layers are gone.
     * As a result the wall rendering could be simplified again too

   Smaller stuff:
     * Raylist is extended and includes the level per ray endpoint now. To be able to render the rays on a per level basis or in different
       colours per level in the minimap. Altered RenderMapRays() to this end.
...
#include "map_16x16 - part 23e.h"

// test objects
#define TEST_OBJ_PERCENTAGE   0.04f    // this percent of *empty* tiles will be used as the nr of test objects
#define MIN_DYNAMIC_OBJS      3        // the first x objects will be dynamic objects
...
void AddBlockBluePrint( char cID, float fH, int nE, int nN, int nW, int nS, int nT, int nB, bool bT = false, bool bP = false, bool bA = false ) {
    BlockBluePrint sTmpBlockBP = { cID, fH, { nE, nN, nW, nS, nT, nB }, bT, bP, bA };
...
void InitBlockBluePrints() {
  AddBlockBluePrint( '.', 0.00f, 0, 0, 0, 0, 0, 0 );

  AddBlockBluePrint( '#', 1.00f, 0, 0, 0, 0, 0, 0 );   // these are indices into the vectors of wall, roof and ceiling sprite pointers
  AddBlockBluePrint( '%', 1.00f, 1, 1, 1, 1, 1, 1 );
  AddBlockBluePrint( '!', 1.00f, 2, 2, 2, 2, 2, 2 );
  AddBlockBluePrint( '@', 1.00f, 3, 3, 3, 3, 3, 3 );
  AddBlockBluePrint( '$', 1.00f, 4, 4, 4, 4, 5, 5, true, false, true );   // door / gate
  AddBlockBluePrint( '&', 1.00f, 5, 5, 5, 5, 5, 5 );
  AddBlockBluePrint( '*', 1.00f, 6, 6, 6, 6, 0, 0, true );                // window
  AddBlockBluePrint( '+', 1.00f, 7, 7, 7, 7, 0, 0, true );                // barred window

  AddBlockBluePrint( 'Q', 0.25f, 0, 0, 0, 0, 0, 0 );
  AddBlockBluePrint( 'H', 0.50f, 0, 0, 0, 0, 0, 0 );
  AddBlockBluePrint( 'T', 0.75f, 0, 0, 0, 0, 0, 0 );
  AddBlockBluePrint( '1', 0.10f, 0, 0, 0, 0, 0, 0 );
  AddBlockBluePrint( '2', 0.20f, 0, 0, 0, 0, 0, 0 );
  AddBlockBluePrint( '3', 0.30f, 0, 0, 0, 0, 0, 0 );
  AddBlockBluePrint( '4', 0.40f, 0, 0, 0, 0, 0, 0 );
  AddBlockBluePrint( '5', 0.50f, 0, 0, 0, 0, 0, 0 );
  AddBlockBluePrint( '6', 0.60f, 0, 0, 0, 0, 0, 0 );
  AddBlockBluePrint( '7', 0.70f, 0, 0, 0, 0, 0, 0 );
  AddBlockBluePrint( '8', 0.80f, 0, 0, 0, 0, 0, 0 );
  AddBlockBluePrint( '9', 0.90f, 0, 0, 0, 0, 0, 0 );
}
...
int   GetBlockBPFaceIx( BlockBluePrint &b, int nFace ) {
    ...
    if (nFace < 0 || nFace >= FACE_NR_OF) {
        std::cout << "GetBlockBPFaceIx() --> index out of range: " << nFace << std::endl;
    } else {
...
public:
    ...
    virtual void Update( float fElapsedTime ) {
        // empty block has no update functionality
    }
...
protected:
    ...
    int x, y, level;    // the tile coordinate and level of this block
...
// ==============================/  class RC_BlockFilled  /==============================

class RC_BlockFilled : public RC_Block {

public:
    RC_BlockFilled() {}

    void SetSpritePtr( int nFaceIx, olc::Sprite *pSprite ) {
        if (nFaceIx < 0 || nFaceIx >= FACE_NR_OF) {
            std::cout << "WARNING: SetSpritePtr() --> face index out of range: " << nFaceIx << std::endl;
        } else {
            pFaceSprites[ nFaceIx ] = pSprite;
        }
    }

    olc::Sprite *GetSpritePtr( int nFaceIx ) {
        if (nFaceIx < 0 || nFaceIx >= FACE_NR_OF) {
            std::cout << "WARNING: GetSpritePtr() --> face index out of range: " << nFaceIx << std::endl;
            return nullptr;
        } else {
            return pFaceSprites[ nFaceIx ];
        }
    }

    void Update( float fElapsedTime ) {
        // regular filled block has no update functionality
    }

    olc::Pixel Sample( int nFaceIx, float sX, float sY ) {
        if (nFaceIx < 0 || nFaceIx >= FACE_NR_OF) {
            std::cout << "WARNING: Sample() --> face index out of range: " << nFaceIx << std::endl;
            return olc::BLANK;
        } else {
            olc::Sprite *pSprite = pFaceSprites[ nFaceIx ];
            if (pSprite == nullptr) {
                std::cout << "WARNING: Sample() --> nullptr sprite ptr encountered for index: " << nFaceIx << std::endl;
                return olc::BLANK;
            } else {
                return pFaceSprites[ nFaceIx ]->Sample( sX, sY );
            }
        }
    }
...
protected:
    olc::Sprite *pFaceSprites[FACE_NR_OF] = { nullptr };   // the sprites for each of the faces
};

// ==============================/  class RC_BlockAnimated   /==============================
...
class RC_BlockAnimated : public RC_BlockFilled {
public:
    RC_BlockAnimated() {}

    void Init( int px, int py, int l, int s, int tw, int th ) {
        x = px;
        y = py;
        level = l;
        state = s;
    ...
    olc::Pixel Sample( int nFaceIx, float sX, float sY ) {
        if (nFaceIx < 0 || nFaceIx >= FACE_NR_OF) {
            ...
            olc::Sprite *faceSpritePtr = pFaceSprites[ nFaceIx ];

            if (faceSpritePtr == nullptr) {
                std::cout << "WARNING: Sample() --> nullptr sprite ptr encountered for face index: " << nFaceIx << std::endl;
                return olc::MAGENTA;
            } else {
                float fx0 = float( ( tileX + sX ) * tileWidth ) / faceSpritePtr->width;
                float fy0 = float( ( tileY + sY ) * tileHight ) / faceSpritePtr->height;

                return faceSpritePtr->Sample( fx0, fy0 );
            }
        }
    }
    ...
    void AddLayer( const std::string &sUserMap, std::vector<olc::Sprite *> vWallTextures,
        ...
        for (int y = 0; y < nMapY; y++) {
            for (int x = 0; x < nMapX; x++) {
                ...
                if (cTileID == '.') {                                // create an empty block for map insertion
                    pBlockPtr = new RC_Block;
                    pBlockPtr->Init( x, y, (int)bMaps.size());

                } else if (refBlock.bAnimated) {                     // create an animated block for map insertion
                    RC_BlockAnimated *auxBlockPtr = new RC_BlockAnimated;
                    auxBlockPtr->Init( x, y, (int)bMaps.size(), ANIM_STATE_CLOSED, 32, 32 );

                    for (int i = FACE_EAST; i <= FACE_BOTTOM; i++) {
                        olc::Sprite *auxSpritePtr = nullptr;
                        switch ( i ) {
                            case FACE_EAST  :
                            case FACE_NORTH :
                            case FACE_WEST  :
                            case FACE_SOUTH : auxSpritePtr = vWallTextures[ refBlock.nFaces[ i ] ]; break;
                            case FACE_TOP   : auxSpritePtr = vRoofTextures[ refBlock.nFaces[ i ] ]; break;
                            case FACE_BOTTOM: auxSpritePtr = vCeilTextures[ refBlock.nFaces[ i ] ]; break;
                        }
                        auxBlockPtr->SetSpritePtr( i, auxSpritePtr );
                    }
                    pBlockPtr = auxBlockPtr;

                } else {                                             // create a filled block for map insertion
                    RC_BlockFilled *auxBlockPtr = new RC_BlockFilled;
                    auxBlockPtr->Init( x, y, (int)bMaps.size());
                    for (int i = FACE_EAST; i <= FACE_BOTTOM; i++) {
                        olc::Sprite *auxSpritePtr = nullptr;
                        switch ( i ) {
                            case FACE_EAST  :
                ...
                // finally put the basic info into the new block ...
                pBlockPtr->SetID(          refBlock.cID          );
                pBlockPtr->SetHeight(      refBlock.fHeight      );
                pBlockPtr->SetTransparent( refBlock.bTransparent );
                pBlockPtr->SetPermeable(   refBlock.bPermeable   );
                // ... and add it to this level
...
public:
    ...
    bool OnUserCreate() override {
        ...
        for (int i = 0; i < nNrTestObjects; i++) {
            ...
            int nRandObj = (i < MIN_DYNAMIC_OBJS) ? 0 : rand() % vObjectSprites.size();
    ...
    bool GetDistancesToWallsPerLevel( int level, float fRayAngle, std::vector<IntersectInfo> &vHitList ) {
        ...
        while (!bOutOfBounds && !bDestCellFound && fDistIfFound < fMaxDistance) {
            ...
            if (bOutOfBounds) {
                ...
                bool bTrnspBlock = auxBlockPtr->IsTransparent();
            ...
            for (int y = 0; y < cMap.Hight(); y++) {
                for (int x = 0; x < cMap.Width(); x++) {
                    ...
                    if (blockPtr == nullptr) {
                        ...
                        nCntEmpty += (blockPtr->IsEmpty() ? 1 : 0);
                        nCntFilled += (!blockPtr->IsEmpty() && !blockPtr->IsAnimated() ? 1 : 0);
                        nCntAnim += (blockPtr->IsAnimated() ? 1 : 0);

                        if (blockPtr->IsAnimated()) {

//std::cout << "Animated block at (" << x << ", " << y << ", " << h << ")" << std::endl;

                            RC_BlockAnimated *pAnimation = (RC_BlockAnimated *)blockPtr;
                            if (bStateChanged) {
                                pAnimation->SetState( nTestAnimState );
                            }
...
//            if (nFrameCntr % 100 == 0) {
//                std::cout << "width = " << glbMapX << ", height = " << glbMapY << ", total = " << glbMapX * glbMapY;
//                std::cout << " #empty = " << nCntEmpty << " #filled = " << nCntFilled << " #animated = " << nCntAnim << " #total = " << nCntEmpty + nCntFilled + nCntAnim << std::endl;
//            }
        }
        ...
        for (int x = 0; x < ScreenWidth(); x++) {
            ...
            if (bMinimap && !vHitPointList.empty()) {
                RayType curHitPoint = { { vHitPointList[0].fHitX, vHitPointList[0].fHitY }, vHitPointList[0].nLevel };
