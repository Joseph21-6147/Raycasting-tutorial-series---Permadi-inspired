...
//
// Implementation of part 23 a - dynamic, animated and wall objects (class RC_Objects introduced)
//
...
//   *  include file "map_demo - part 23a.h" - contains the map definitions

/* Short description
   -----------------
   This implementation is a follow up of implementation part 22 d. See the description of that implementation as well (and check on
   the differences between that cpp file and this one).

     * Put the map definitions in a separate file for practical purposes. This one uses map_demo - part 23a.h
     * Elaborated the object struct into a class RC_Object, having initializer constructor, setters, getters, printers and an update method.
       For now the update method just bounces off map boundaries. No collision detection yet.
     * Introduced test slice - a magenta aiming line that can be triggered with G key, aimed with F1 and F2, and along which the hit list
       is output upon pressing T.
     * Small stuff
         + Made the placement of random objects in the screen a bit more controlled.
         + Added speedup or slow down options for movement and rotations
         + altered the interface of the lambda's to get the roof, ceiling and floor samples. Since the distance to the sample point is
           calculated there, I can use that distance for the depth buffer.
...
// test objects
#define NR_TEST_OBJECTS      100      // switch off to focus on wall rendering
        ...
        for (int y = 0; y < nMapY; y++) {
            for (int x = 0; x < nMapX; x++) {
                switch (sMap[ y * nMapX + x ]) {
                    ...
                    default         : std::cout << "ERROR: AddLayer() --> unknown sMap value in layer " << (int)fMaps.size() << " at ( " << x << ", " << y << "): " << sMap[ y * nMapX + x ] << std::endl;
                }
            }
        }
...
public:
    ...
    void SetDistance( float fD ) { distance = fD; }
    void SetAngle(    float fA ) { angle    = fA; }
    ...
    void SetVX( float fVX ) { vx = fVX; }
    void SetVY( float fVY ) { vy = fVY; }

    float GetVX() { return vx; }
    float GetVY() { return vy; }

    void Update( int lowX, int lowY, int hghX, int hghY, float fElapsedTime ) {
        if (!bStatic) {
            float newX = x + vx * fElapsedTime;
            float newY = y + vy * fElapsedTime;
            if (newX >= float( lowX ) && newX < float( hghX )) {
                x = newX;
            } else {
                vx = -vx;
            }
            if (newY >= float( lowY ) && newY < float( hghY)) {
                y = newY;
            } else {
                vy = -vy;
            }
    ...
    bool OnUserCreate() override {
        ...
        for (int i = 0; i < NR_TEST_OBJECTS; i++) {
            ...
            if (bMakeDynamic) {
                ...
                tmpObj.SetVX( float_rand_between( -2.0f, 2.0f ));
                tmpObj.SetVY( float_rand_between( -2.0f, 2.0f ));
...
// ==============================/   Mini map rendering prototypes   /==============================
    ...
    bool OnUserUpdate( float fElapsedTime ) override {
        ...
        for (auto &elt : vListObjects) {
            elt.Update( 0, 0, cMap.Width(), cMap.Hight(), fElapsedTime );
        }
        ...
        for (int x = 0; x < ScreenWidth(); x++) {
            ...
            auto get_floor_sample = [=]( int px, int py ) -> olc::Pixel {
                ...
                return generic_sampling( fFloorProjDistance, pFloorSprite );
            };
            ...
            auto get_roof_sample = [=]( int px, int py, int nLevel, float fHeightWithinLevel, float &fRoofProjDistance ) -> olc::Pixel {
                ...
                // call the generic sampler to work out the rest
                return generic_sampling( fRoofProjDistance, pRoofSprite );
            };
            ...
            auto get_ceil_sample = [=]( int px, int py, int nLevel, float fHeightWithinLevel, float &fCeilProjDistance ) -> olc::Pixel {
                ...
                // call the generic sampler to work out the rest
                return generic_sampling( fCeilProjDistance, pCeilSprite );
            };
        ...
        for (auto &object : vListObjects) {
            ...
            object.SetDistance( sqrtf( fVecX * fVecX + fVecY * fVecY ));
            ...
            if (fObjA >   PI) fObjA -= 2.0f * PI;
            object.SetAngle( fObjA );

            // experiment
            object.SetDistance( object.GetDistance() * lu_cos( fObjA ));
        }
        ...
        for (auto &object : vListObjects) {
            // can object be seen?
            float fObjDist = object.GetDistance();
