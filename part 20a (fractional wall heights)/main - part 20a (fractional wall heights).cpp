...
//
// Implementation of part 20 a - fractional wall heights
//
...
/* Short description
   -----------------
   This implementation is a follow up of implementation part 19 b. See the description of that implementation as well (and check on
   the differences between that cpp file and this one).

   It was remarkably easy to alter the code to support fractional wall heights. The only things I needed to change were:
     * adapt nMap (an array of ints denoting the heights in the map) to fMap (an array of floats)
     * create constants for fractional height blocks and initialize the map with it
     * adapt all variables that are used to calculate the height of walls and the projection of wall slices on the screen
   That's it :)

   I also put in mini map rendering, and toggles to turn the mini map and debug info on or of.

   Oh and I adapted the map for testing purposes, and enlarged the window for better visual effect.
