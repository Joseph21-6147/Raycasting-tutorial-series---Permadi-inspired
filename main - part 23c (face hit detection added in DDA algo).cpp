...
//
// Implementation of part 23 c - dynamic, animated and wall objects (detection which face was hit in DDA algo)
//
...
//   *  include file "map_16x16 - part 23c.h" - contains the map definitions and the sprite file names

/* Short description
   -----------------
   This implementation is a follow up of implementation part 23 b. See the description of that implementation as well (and check on
   the differences between that cpp file and this one).

   The changes in this code file are relatively limited:
     * Introduced 6 face constants for storing in the hitlist which face was hit. Had to change the type IntersectInfo and the DDA
       function (GetDistancesToWallsPerLevel()) for this.
     * This makes rendering wall segments a bit easier, since the hit face is coming with the hit list, and you don't have to
       figure it out anymore. This also saves an atan2f() call :)
        ...
        for (int x = 0; x < ScreenWidth(); x++) {
            ...
            for (auto &hitRec : vHitPointList) {
                ...
                if (hitRec.fHeight > 0.0f) {
                    ...
                    // render wall segment - first get x sample coordinate from face hit info
                    float fSampleX;
                    switch (nFaceHit) {
                        case FACE_SOUTH:
