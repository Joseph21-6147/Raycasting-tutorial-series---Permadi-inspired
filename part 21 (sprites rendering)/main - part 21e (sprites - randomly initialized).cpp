...
//
// Implementation of part 21 e - introducing sprites (billboards): generated randomly
//
...
/* Short description
   -----------------
   This implementation is a follow up of implementation part 21 d. See the description of that implementation as well (and check on
   the differences between that cpp file and this one).

     * Just added some more objects and randomly initialisation of them.
...
private:
    ...
    olc::Sprite *pObjectSprite[2] = { nullptr };
...
#define FLOOR_3QRTR 'T'    //                 3/4
    ...
    bool OnUserCreate() override {
        ...
        pObjectSprite[0] = load_sprite_file(              "tree 100x100.rbg.png" ); bSuccess &= (pObjectSprite != nullptr);
        pObjectSprite[1] = load_sprite_file( "elf-girl_stationary-front.rbg.png" ); bSuccess &= (pObjectSprite != nullptr);
        ...
        for (int i = 0; i < 20; i++) {
            ...
            int nRandObj = rand() % 2;
            int nRandSize = (nRandObj == 0) ? rand() % 45 + 5 : rand() % 13 + 2;
...
//                            }
                            if (pSample != olc::BLANK) {
                                DrawDepth( fObjDist, nObjColumn, fObjCeiling + fy, pSample );
                            }
                        }
                    }
                }
