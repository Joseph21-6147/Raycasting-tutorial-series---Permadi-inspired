...
//
// Implementation of part 25 c - first working version of portals
//
...
/* Short description
   -----------------
   This implementation is a follow up of implementation part 25 b. See the description of that implementation as well (and check on
   the differences between that cpp file and this one).

   Summary of the changes I made:
      * main.cpp
          - changed signature of InitMap()
          - extended the number of object sprites, meaning that I had to adapt my random object initialisation code in OnUserCreate()
          - introduced raw distances (not corrected) and corrected distances (corrected for fish eye effect) in IntersectInfo struct, and applied where necessary.
          - added (test code for) a third map
          - added player teleport code in OnUserUpdate()
          - added code to make portal map cell visible as such in rendering of mini map
      * map_16x16.h
          - changed some texture files and added more object sprites (but that's functionally not really relevant)
          - Added the concept of portal descriptor records, having the info from which map cell (map id, xyz coordinate in that map) to which other map cell the portal will teleport
      * RC_Face_blueprints.cpp
          - added a face to be used as the face of a portal map cell
      * RC_Map
          - Added a list of portal descriptors per RC_Map object.
          - Implemented the initialisation of portal map cells in AddLayer()
          - Added a search method for obtaining the portal descriptor at a specified map cell
      * RC_Face
          - definition of portal descriptor struct
          - added a derived class RC_MapCellPortal, and implemented method IsPortal() in all RC_MapCell classes
      * RC_MapCell_blueprint
          - added a flag to signal whether map cell blueprint is a portal type map cell
...
public:
    ...
    bool OnUserCreate() override {
        ...
        for (int i = 0; i < nNrTestObjects; i++) {
            ...
            int nRandSize;
            switch (nRandObj) {
                case 0:                                // this is an elf girl, make dynamic
                    bMakeDynamic = true;  nRandSize = rand() %  3 + 3;
                    break;
                case 1: case 2:                        // these are stationary objects of fixed size
                    bMakeDynamic = false; nRandSize =  6;
                    break;
                case  3: case  4: case 5: case  6:     // these are bushes
                case  7: case  8: case 9: case 10:
                case 11: case 12:
                    bMakeDynamic = false; nRandSize = rand() % 8 + 2;
                    break;
                default:
...
// ==============================/   Mini map rendering prototypes   /==============================
    ...
    bool OnUserUpdate( float fElapsedTime ) override {
        ...
        // test code - switch to map 0 (or 1 or 2) upon pressing key '0' (or '1' or '2')
        ...
        for (int x = 0; x < ScreenWidth(); x++) {
            ...
            std::sort(
                ...
                []( IntersectInfo &a, IntersectInfo &b ) {
                    return (a.fDistFrnt_corr > b.fDistFrnt_corr) ||
                           (a.fDistFrnt_corr == b.fDistFrnt_corr && a.nLayer < b.nLayer);
                }
            );
            ...
            for (auto &hitRec : vHitPointList) {
                ...
                if (hitRec.fHeight > 0.0f) {
                    ...
                    fFrntDistance     = hitRec.fDistFrnt_corr;
...
void MyRayCaster::RenderProcessInfo() {
    ...
    DrawString( nStartX + 5, nStartY +  45, "Acive map      = " + std::to_string( nActiveMap                      ), COL_HUD_TXT );
